...
By default, Meteostat substitutes missing observations with model data. Model data may include weather forecasts to improve the user experience when working with near-real-time data. You can opt out of model data if you require observation-only results.
...
Meteostat is not a public or governmental service, and it is not affiliated with _Meteosat_. We collect data from national weather services such as NOAA and DWD. Meteostat is an independent initiative that relies on donations, sponsorship, and other funding sources.
...
We are grateful for all forms of support. Learn more about [contributing](/contribute) and [donating](/donate).
